...
| Step | Status |
|------|--------|
| Route runtime imports through workspace crates (`spanda-ast`, `spanda-runtime`, …) instead of `crate::` | **In progress** — kernel modules + **`spanda-comm`** (`CommBus`, `InMemoryCommBus`, comm safety); **remaining:** `ai`, `safety`, `transport`, `hal`, `providers` bootstrap, `concurrency`, `error` |
