...
| **Distributed decisions** | Brain/spinal-cord/reflex layers; `decision_tree`, `offline_policy`, entity `local_decision_authority`; runtime policy gates (offline forbid, central approval escalation); live v3 trace emission; CLI `spanda decision *`; REST `/v1/decisions*` + `/v1/decisions/traces`; gRPC ListDecisions/ListDecisionTraces; Control Center Decisions tab with live trace panel; `examples/showcase/distributed_decisions/` |
