...
| **Platform maturity (Phase D)** | Verify-time tamper check (`spanda tamper-check`), integrity (`spanda integrity`), decision trace explain (`spanda explain decision`), runtime policy (`--enforce-policy`) — **Experimental** |
