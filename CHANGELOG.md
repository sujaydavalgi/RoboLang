...
- **Configuration drift detection:** `detect_config_drift` / `ConfigDriftReport` in `spanda-config`; CLI `spanda drift` and `spanda config drift --baseline <dir>`; `spanda readiness --baseline` for drift gates; docs [drift-detection.md](docs/drift-detection.md).
