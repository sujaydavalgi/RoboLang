...
- **Composite trust in gates and scorecard:** `spanda deploy gate` adds `composite_trust` and `secure_boot` gates; scorecard security pillar blends threat risk and composite trust; `spanda explain` includes `composite_trust` section; `spanda demo maturity` runs program trust.
