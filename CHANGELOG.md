...
- **Differentiation NOW (v1):** `spanda-contract`, `spanda-explain`, `spanda-decision` crates; CLI `spanda contract verify`, `spanda explain`, `spanda audit decisions`, `spanda safety-coverage`, `spanda recovery-coverage`; `spanda demo differentiation`; `examples/showcase/differentiation/warehouse.sd`; `scripts/differentiation_smoke.sh` (wired into `showcase_smoke.sh`).
