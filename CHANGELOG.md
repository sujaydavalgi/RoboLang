...
- **Device identity mapping:** `[[devices]]` registry with network/bus identity fields (`ip`, `mac`, `serial`, `endpoint`, `protocol`, `can_id`, `trust_level`, …); `DeviceRegistry` on `ResolvedSystemConfig`; duplicate IP/MAC/serial validation; redundant failover rules; CLI `spanda device discover|inspect`, `spanda network scan`, `spanda config report --network`; readiness connectivity issues from device identity gaps.
