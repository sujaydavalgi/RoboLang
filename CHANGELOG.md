...
- **TypeScript mirror alignment (Phase 5a):** Split compile from run (`src/cli/run-program.ts`); inject `TelemetrySink`, `SecurityRuntime`, `ProviderRuntime`, and `AdapterRuntime` at the CLI boundary with platform-service bridges; closed architecture waivers `TS-ARCH-002`–`TS-ARCH-006` and `TS-ARCH-021`–`TS-ARCH-028` (24 TypeScript waivers remain).
