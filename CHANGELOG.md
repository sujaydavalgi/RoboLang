...
- **Lean-core architecture:** provider trait contracts in `crates/spanda-core/src/providers/` (`SensorProvider`, `TransportProvider`, `NavigationProvider`, `VisionProvider`, `FleetProvider`, and 10 more); `ProviderRegistry` for runtime registration; `TransportAdapterProvider` bridge for legacy `TransportAdapter`; module classification audit table; 17 official package scaffolds under `packages/registry/` (`spanda-gps`, `spanda-wifi`, `spanda-ble`, `spanda-cellular`, `spanda-dds`, `spanda-slam`, `spanda-nav`, `spanda-opencv`, `spanda-yolo`, `spanda-moveit`, `spanda-gazebo`, `spanda-webots`, `spanda-fleet`, `spanda-ota`, `spanda-maintenance`, `spanda-ledger`, `spanda-cloud`); extended `framework_packages()` registry metadata; docs: [lean-core.md](docs/lean-core.md), [provider-interfaces.md](docs/provider-interfaces.md), [official-packages.md](docs/official-packages.md), [security-architecture.md](docs/security-architecture.md), [lean-core-roadmap.md](docs/lean-core-roadmap.md); compatibility shims retained for `transport_*`, `connectivity_positioning`, fleet, and deploy modules; **Phase 2 complete:** `Interpreter` auto-bootstraps `ProviderRegistry`; `bootstrap_providers_for_packages()` scopes transports to installed deps; `sync_comm_bus_for_official_packages()` connects comm-bus adapters; `load_official_packages_for_project()` / CLI `run_options_for_file()` wire `spanda run`/`sim`/`fleet run` to project lockfile; `spanda install` reports official packages; TS fleet orchestrate import fix; TypeScript providers mirror
