...
- **Phase 8 native interpreter:** `cargo build -p spanda-interpreter` compiles the runtime tree natively; `run_program` and `run_tests_with_registry` live in `spanda-interpreter`; `spanda-core` depends one-way on `spanda-interpreter` and re-exports runtime/run APIs; `#[path]` shim removed.
