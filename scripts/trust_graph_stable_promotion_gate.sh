...
PROGRAM="$ROOT/examples/showcase/trust_graph/rover.sd"
SOAK_FILE="${SPANDA_TRUST_GRAPH_FIELD_SOAK_START_FILE:-$ROOT/.spanda/trust-graph-field-soak-start.txt}"
MIN_DAYS="${SPANDA_TRUST_GRAPH_FIELD_SOAK_MIN_DAYS:-30}"

echo "== Trust graph stable promotion gate =="
if [[ "${SPANDA_TRUST_GRAPH_SKIP_SOAK:-1}" != "1" ]]; then
  ...
  (( ELAPSED_DAYS >= MIN_DAYS )) || { echo "Trust graph soak incomplete" >&2; exit 1; }
else
  echo "Skipping field soak (SPANDA_TRUST_GRAPH_SKIP_SOAK=1)"
fi

if [[ "${SPANDA_TRUST_GRAPH_SKIP_SMOKE:-0}" != "1" ]]; then
  "$ROOT/scripts/trust_graph_smoke.sh"
else
  echo "Skipping smoke (SPANDA_TRUST_GRAPH_SKIP_SMOKE=1)"
fi
...
sleep 2
body="$(curl -sf --max-time 15 "http://${BIND}/v1/analytics/trust-graph")"
echo "$body" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert d.get("version")=="v1", d'
echo "Trust graph stable promotion gate passed."
