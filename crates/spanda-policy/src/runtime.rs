...
pub fn build_runtime_policy_monitor(
    ...
    // Extract motion-related policy rules for interpreter enforcement.
    //
    // Parameters:
    ...
    //
    // Returns:
    // Runtime policy monitor or not-found error.
    //
    // Options:
    // None.
    //
    // Example:
    // let monitor = build_runtime_policy_monitor(&program, "WarehousePolicy")?;
...
pub fn check_runtime_policy_motion(
    ...
    // Enforce max-speed and operation-hours rules before actuator motion.
    //
    // Parameters:
    // - `monitor` — compiled runtime policy
    // - `linear_mps` — requested linear speed in meters per second
    //
    // Returns:
    // Ok when motion is allowed, or a runtime policy violation.
    //
    // Options:
    // `RuntimePolicyMonitor::clock_override` for deterministic tests.
    //
    // Example:
    // check_runtime_policy_motion(&monitor, 0.8)?;
