...
use crate::ast::{Program, RobotDecl, TopicDecl};
...
pub fn security_check(source: &str) -> Result<SecurityReport, crate::SpandaError> {
...
pub fn security_audit(source: &str) -> Result<SecurityReport, crate::SpandaError> {
    let tokens = crate::lexer::tokenize(source)?;
    let program = parse(tokens)?;
...
fn analyze_robot(robot: &RobotDecl, report: &mut SecurityReport) {
    ...
    for service in services {
        let crate::ast::ServiceDecl::ServiceDecl { secure, span, .. } = service;
    ...
    for action in actions {
        let crate::ast::ActionDecl::ActionDecl {
            secure,
            span,
            action_type,
            ..
    ...
    if let Some(perms) = permissions {
        let crate::foundations::PermissionsDecl::PermissionsDecl { capabilities, .. } = perms;
...
fn secret_is_crypto_material(secret: &SecretDecl) -> bool {
    ...
    name.contains("key")
        || name.contains("cert")
        || matches!(source, crate::foundations::SecretSourceDecl::File { .. })
}
